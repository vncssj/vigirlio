...
class GymMemberController extends AppController
{
	...
	public function addMember()
	{
		...
		if ($this->request->is("post")) {
			...
			$ds = $this->GYMFunction->check_valid_extension($this->request->data['documento']['name']);
			if ($ext != 0) {

				$this->request->data['member_id'] = $member_id;
				$image = $this->GYMFunction->uploadImage($this->request->data['image']);
				$documento = $this->GYMFunction->uploadPdf($this->request->data['documento']);
				$this->request->data['documento'] = (!empty($documento)) ? $documento : "documento.pdf";
				...
				$this->Flash->error(__("Extensão de arquivo inválida, tente novamente."));
	...
	public function attMember($id)
	{
		$this->set("edit", true);
		$this->set("title", __("Relatório de Acompanhamento"));
		$this->set("eid", $id);
		...
		if ($this->request->is("post")) {
			...
			if (1 == 1) {
				...
				if ($this->GymMember->save($update)) {
					...
					//die;
					return $this->redirect(["action" => "viewMember/$id"]);
				} else {
					if ($update->errors()) {
	...
	public function editMember($id)
	{
		...
		if ($session["id"] != $data["id"] && $session["role_name"] != 'administrator') {
			echo $this->Flash->error("Não se esgueirando! ;( ");
			return $this->redirect(["action" => "memberList"]);
		}
		...
		if ($this->request->is("post")) {
			...
			if ($ext != 0) {
				...
				if ($documento != "") {
					$this->request->data['documento'] = $documento;
					unlink(WWW_ROOT . "/upload/" . $data['documento']);
				} else {
					unset($this->request->data['documento']);
				}
				/* $this->request->data['image'] = $image ; */
	...
	public function deleteMember($id)
	{
		...
		if ($this->GymMember->delete($row)) {
			$this->Flash->success(__("Sucesso! Registro apagado com sucesso."));
	...
	{
		$weight_data["data"] = $this->GYMFunction->generate_chart("Peso", $id);
		$weight_data["option"] = $this->GYMFunction->report_option("Peso");
		$this->set("weight_data", $weight_data);

		$height_data["data"] = $this->GYMFunction->generate_chart("Panturrilha", $id);
		$height_data["option"] = $this->GYMFunction->report_option("Panturrilha");
		$this->set("height_data", $height_data);

		$thigh_data["data"] = $this->GYMFunction->generate_chart("Coxa", $id);
		$thigh_data["option"] = $this->GYMFunction->report_option("Coxa");
		$this->set("thigh_data", $thigh_data);

		$chest_data["data"] = $this->GYMFunction->generate_chart("Peito", $id);
		$chest_data["option"] = $this->GYMFunction->report_option("Peito");
		$this->set("chest_data", $chest_data);

		$waist_data["data"] = $this->GYMFunction->generate_chart("Cintura", $id);
		$waist_data["option"] = $this->GYMFunction->report_option("Cintura");
		$this->set("waist_data", $waist_data);

		$arms_data["data"] = $this->GYMFunction->generate_chart("Braco", $id);
		$arms_data["option"] = $this->GYMFunction->report_option("Braco");
		$this->set("arms_data", $arms_data);

		$fat_data["data"] = $this->GYMFunction->generate_chart("Gordura", $id);
		$fat_data["option"] = $this->GYMFunction->report_option("Gordura");
	...
	public function activateMember($aid)
	{
		...
		if ($this->GymMember->save($row)) {
			...
			$message = "{$header}<br>Membro ID: $member_id\n<br>Plano: $membership\n<br>Status do Plano: $membership_status\n";
			$message .= "<br>Associação Válida De: $membership_valid_from\n";
			$message .= "<br>Membro válido até: $membership_valid_to\n\n<br>Cumprimentos\n$sys_name";
			...
			if ($ext == 'csv') {
				...
				if ($this->request->data['import']['size'] > 0) {
					...
					while (($getData = fgetcsv($file, 10000, ",")) !== FALSE) {
						...
						$sql = "INSERT INTO gym_member	(activated,role_name,member_id,first_name,middle_name,last_name,member_type,gender,birth_date,address,city,state,zipcode,mobile,email,username,password,image,documento,assign_staff_mem,selected_membership,membership_status,membership_valid_from,membership_valid_to,created_date) VALUES('" . $row['activated'] . "','" . $row['role_name'] . "','" . $row['member_id'] . "','" . $row['first_name'] . "','" . $row['middle_name'] . "','" . $row['last_name'] . "','" . $row['member_type'] . "','" . $row['gender'] . "','" . $row['birth_date'] . "','" . $row['address'] . "','" . $row['city'] . "','" . $row['state'] . "','" . $row['zipcode'] . "','" . $row['mobile'] . "','" . $row['email'] . "','" . $row['username'] . "','" . $row['password'] . "','" . $row['image'] . "','" . $row['documento'] . "','" . $row['assign_staff_mem'] . "','" . $row['selected_membership'] . "','" . $row['membership_status'] . "','" . $row['membership_valid_from'] . "','" . $row['membership_valid_to'] . "',date('Y-m-d'))";
						...
						if ($conn->execute($sql)) {
							...
							$this->Flash->error(__("Arquivo não importado, tente novamente."));
							return $this->redirect(["action" => "memberList"]);
						}
					}
					if ($count > 0) {
						$count = $count - 1;
						$this->Flash->success(__("Importação de arquivo com sucesso e $count Record Generated."));
						return $this->redirect(["action" => "memberList"]);
					}
					fclose($file);
				} else {
					echo '<script>alert("Ficheiro vazio")</script>';
					$this->Flash->error(__("File is empty"));
					return $this->redirect(["action" => "memberList"]);
				}
			} else {
				$this->Flash->error(__("Incompatibilidade de extensão de arquivo. por favor carregue o arquivo csv"));
				return $this->redirect(["action" => "memberList"]);
			}
		}
	}
	public function isAuthorized($user)
	{
		...
		$members_actions = ["viewMember", "attMember", "memberList", "viewAttendance"];
		$staff_acc_actions = ["memberList", "attMember", "viewMember", "viewAttendance"];
		switch ($role_name) {
			case "member":
