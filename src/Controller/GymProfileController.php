		...
		if($this->request->is("post") )
		{
			...
			{			
				...
				if($curr_pass)
				{
					...
					{
						$this->Flash->error(__("Erro! A nova senha e a confirmação da senha não correspondem. Tente novamente."));
					}else{
						...
						{
							$this->Flash->success(__("Sucesso! Senha atualizada com sucesso"));
						}
					}
				}else{
					$this->Flash->error(__("Sucesso! Informação atualizada com sucesso."));
				}
			...
			if(isset($this->request->data["profile_save_change"]))
			{
				...
				if($count == 0)
				{
					...
					if($this->GymProfile->GymMember->save($update_row))
					{
						$this->Flash->success(__("Sucesso! Informação atualizada com sucesso"));
						return $this->redirect(["action"=>"viewProfile"]);
					}
				}else{
					$this->Flash->error(__("Erro! Não atualize. Por favor, tente novamente."));
				}
			}			
		}
	}
