...
Class ClassBookingController  extends AppController
{
	...
	{		
		...
		if($this->request->is('post')){
			 ...
			 && $data['booking_date'] != '')
			{
				if($data['payment_by'] == 'Stripe'){
					...
					if(!empty($this->request->data['stripeToken'])) {
						  	...
						  	) 
						{
							...
							$this->Flash->error(__("Erro! Algo está errado."));
							return $this->redirect(["action"=>"index"]);
						}
					}
					...
					//debug($booking);die;
					if($booking != NULL){
						$this->Flash->error(__("Erro! Essa aula já é de sua autoria, agora você não pode agendar essa aula como demo."));
						return $this->redirect(["action"=>"index"]);
					}else{
						$row = $this->ClassBooking->newEntity();
				...
				$this->Flash->error(__("Erro! Preencha todas as informações."));
						return $this->redirect(["action"=>"index"]);
			}
		}
	}
	...
	public function regComplete()
	{
		...
		echo "<br><p><i><strong>Sucesso!</strong> Registro concluído com sucesso.</i></p>";
		echo "<p><i><a href='{$this->request->base}/Users'>Clique aqui</a> para redirecionar para a página de login.</i></p>";
	}
