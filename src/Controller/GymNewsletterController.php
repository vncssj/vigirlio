...
class GymNewsletterController extends AppController {
		...
		if($this->request->is("post")) {
			if(!$edit) {
				...
				if($this->GymNewsletter->save($row)) {
					$this->Flash->success(__("Sucesso! Chave API salva com sucesso."));
				...
				if($this->GymNewsletter->save($row)) {
					$this->Flash->success(__("Sucesso! Chave API editada com sucesso."));
				}
			}
		}		
	}
	...
	public function syncMail() {
		...
		if($this->request->is("post")) {
			...
			if(!empty($subcsriber_emil)) {
				...
				$this->Flash->success(__("Sucesso! Sincronização de correio concluída com sucesso."));
			}
		}
	}
	...
	public function Campaign() {
		...
		if($this->request->is("post")) {
			...
			if ($api->errorCode) {
				$this->Flash->error(__("Testes de campanha não enviados!"));
			}else {
				$this->Flash->success(__("Testes de campanha enviados!"));
			}
		}		
	}
