...
class GYMfunctionComponent extends Component
{
	...
	public function generate_chart($type, $mid)
	{
		...
		switch ($type) {
			case "Peso":
				$report_type_array[] = array('date', 'Peso ');
				if (isset($all_data['Peso']) && !empty($all_data['Peso'])) {
					foreach ($all_data['Peso'] as $r) {
						$report_type_array[] = array($r['date'], (int)$r['result']);
					}
				}
				break;
			case "Coxa":
				$report_type_array[] = array('date', 'Coxa	');
				if (isset($all_data['Coxa']) && !empty($all_data['Coxa'])) {
					foreach ($all_data['Coxa'] as $r) {
						$report_type_array[] = array($r['date'], (int)$r['result']);
					}
				}
				break;
			case "Panturrilha":
				$report_type_array[] = array('date', 'Panturrilha	');
				if (isset($all_data['Panturrilha']) && !empty($all_data['Panturrilha'])) {
					foreach ($all_data['Panturrilha'] as $r) {
						$report_type_array[] = array($r['date'], (int)$r['result']);
					}
				}
				break;
			case "Peito":
				$report_type_array[] = array('date', 'Peito	');
				if (isset($all_data['Peito']) && !empty($all_data['Peito'])) {
					foreach ($all_data['Peito'] as $r) {
						$report_type_array[] = array($r['date'], (int)$r['result']);
					}
				}
				break;
			case "Cintura":
				$report_type_array[] = array('date', 'Cintura	');
				if (isset($all_data['Cintura']) && !empty($all_data['Cintura'])) {
					foreach ($all_data['Cintura'] as $r) {
						$report_type_array[] = array($r['date'], (int)$r['result']);
					}
				}
				break;
			case "Braco":
				$report_type_array[] = array('date', 'Braco	');
				if (isset($all_data['Braco']) && !empty($all_data['Braco'])) {
					foreach ($all_data['Braco'] as $r) {
						$report_type_array[] = array($r['date'], (int)$r['result']);
					}
				}
				break;
			case "Gordura":
				$report_type_array[] = array('date', 'Gordura	');
				if (isset($all_data['Gordura']) && !empty($all_data['Gordura'])) {
					foreach ($all_data['Gordura'] as $r) {
						$report_type_array[] = array($r['date'], (int)$r['result']);
					}
				}
				break;
	...
	*/
		...
		foreach ($data as $member) {
			...
			if ($member["alert_sent"] == 0) {
				...
				if (!empty($membership)) {
					...
					$substituir = [$member_name];
					$reminder_message = $this->getSettings("reminder_message");
					$reminder_message = str_replace($search, $replace, $reminder_message);

					$reminder_message = '<p style="vertical-align: middle;font-size: 20px;color: #fff;font-weight: 700;display:block;background: #000;font-size: 16px;">
					<img src="' . $url . $logo . '" alt="' . $sys_name . '" height="100" style="display: inline-block;vertical-align: middle;"> ' . $sys_name . '</p>' . $reminder_message;

					$alerta_msg = $this->getSettings("mensagem_alerta");
					$alerta_msg = str_replace($search, $substituir, $alerta_msg);
					$expiry_date = $member["membership_valid_to"]->format("Y-m-d");

					$reminder_days = (int)$this->getSettings("reminder_days");
					$dias_alerta = (int)$this->getSettings("dias_alerta");
					$mail_date = date('Y-m-d', (strtotime("-{$reminder_days} day", strtotime("{$expiry_date}"))));

					$mail_data = date('Y-m-d', (strtotime("-{$dias_alerta} day", strtotime("{$expiry_date}"))));
					// 	$mail_data = date('Y-m-d',(strtotime ( "-{$dias_alerta} day" , strtotime ( $expiry_date) ) ));
					...
					if ($curr_date > $mail_date && $curr_date <= $last_date) {
						...
						$email->from([$sys_email => $sys_name])
							->to($to)
							->emailFormat('html')
							->subject('Alerta de lembrete de associação | ' . $sys_name)
							->send($reminder_message);
						// } catch (\Exception $e) {
						// return $e->getMessage();
						// }

						//@mail($to,_("Alerta de lembrete de associação!"),$reminder_message,$headers);


						$user_ids[] = $member["id"];
					}

					//Envio de lembrete de relatório
					if ($curr_date > $mail_data && $curr_date <= $last_date) {
						...
						$email->from([$sys_email => $sys_name])
							->to($to)
							->emailFormat('html')
							->subject('Alerta de lembrete de relatório!' . $sys_name)
							->send($reminder_message);
						//@mail($to,_("Alerta de lembrete de relatório!"),$alerta_msg,$headers);
						$user_ids[] = $member["id"];
					}
				}
			}
	...
	public function sendAlertEmailToAdmin()
	{
		...
		$subject = "{$sys_name} Lista de usuários de membros expirados";
		...
		$message = "{$header}<br>Olá $display_name,<br><br>
			A associação do usuário listado abaixo expira em breve, entre em contato com esses usuários para renovar a associação.<br>";

		$message .= "<table border=1 cellpadding=10 style='font-family:Poppins,sans-serif;border-spacing:0px;'>
					<caption style='font-size: 22px;font-weight: 700;'>Detalhes do membro </caption>
					<thead>
						<th>No</th>
						<th>Nome</th>
						<th>Email</th>
		...
		$regards = 'Saudações de,<br>
					Virgilio de Souza | Consultoria Esportiva.';
		...
		if (!empty($user_ids)) {
			...
			$email->from([$sys_email => $sys_name])
				->emailFormat('html')
				->to($to)
				->subject('Alerta de lembrete de sócios! | ' . $sys_name)
				->send($message);
			//@mail($to,_("Alerta de lembrete de sócios!"),$message,$headers);
	...
	public function index()
	{
		...
		$to = "suporte@virgiliodesouza.com";
		mail($to, "Meu assunto", $msg);
	...
	public function check_valid_extension($filename)
	{
		...
		if ($filename != '') {
			...
			$valid_extension = ['gif', 'png', 'pdf', 'jpg', 'jpeg', "", 'JPG', 'GIF', 'PNG', 'PDF', 'JPEG'];
	...
	public function sendworkout($id = null)
	{
		...
		foreach ($DailyWorkoutData as $dailydata) {
			...
			$mailSubject = "Lembrete de treino!";
			$header = '<p style="vertical-align: middle;font-size: 20px;color: #fff;font-weight: 700;display:block;background: #000;font-size: 16px;">
						<img src="' . $url . $logo . '" alt="' . $sys_name . '" height="100" style="display: inline-block;vertical-align: middle;"> ' . $sys_name . '</p>';

			$mailMessage = "{$header}<br>Querido $memberName
							Hoje Seu Treino Completo.
							Cumprimentos,<br>
							$GYMName";
			...
			if ($_SERVER['SERVER_NAME'] == 'localhost' || $_SERVER['SERVER_NAME'] == '192.168.1.109') {
				...
				$email->from([$sys_email => $sys_name])
					...
					->subject($mailSubject)
					->emailFormat('html')
					->send("Querido $memberName
							Hoje seu treino está completo.
							Cumprimentos,
							$GYMName");
			} else {
