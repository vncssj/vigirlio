...
{  
	...
	public function assignWorkout()
    {
		...
		if($this->request->is("post"))
		{
			...
			//$image = $this->GYMFunction->uploadPdf($this->request->data['description']);
			$ext = $this->GYMFunction->check_valid_extension($this->request->data['sheet']['name']);
			if($ext != 0)
			{
				$image = $this->GYMFunction->uploadImage($this->request->data['sheet']);
				if($image != "")
				{
					$this->request->data['sheet'] = $image;
					unlink(WWW_ROOT."/upload/".$data['sheet']);
				}else{
					unset($this->request->data['sheet']);
				}
			}
			...
			$insert["sheet"] =$this->request->data["sheet"];
			...
			$this->request->data['sheet'] = (!empty($image)) ? $image : "Thumbnail-img.png";
			...
			if($save)
			{
				...
				{
					$this->Flash->error(__("Erro! Não foi possível salvar os dados dos dias de treino. Tente novamente."));
					return $this->redirect(["action"=>"workoutLog"]);
				}
			...
			{
				$this->Flash->error(__("Erro! Não foi possível salvar o registro. Tente novamente."));
			}
		}
	}
	...
	public function viewWorkouts($id)
    {
		...
		$data = $this->GymAssignWorkout->find()->where(["GymAssignWorkout.user_id"=>$id])
			->select(["GymAssignWorkout.start_date","GymAssignWorkout.end_date","GymAssignWorkout.description","GymAssignWorkout.sheet"]);
			...
			foreach($value as $k=>$v)
			{
				if($k == "GymWorkoutData"){
					...
					if($wid != "")
					{
						...
						$work_outdata[$wid]["description"]= $value["description"];
						$work_outdata[$wid]["sheet"]= $value["sheet"];
						$work_outdata[$wid][]=$v;
					}
				}	
