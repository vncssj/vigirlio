	...
	<div class="col-md-12 box box-default">		
		<div class="box-header">
			<section class="content-header">
				<h1>
					...
					<?php echo __("Lista de Pagamentos");?>
					<small><?php echo __("Pagamentos");?></small>
				</h1>
				 <?php
				if($session["role_name"] == "administrator" || $session["role_name"] == "staff_member")
				{ ?>
				<ol class="breadcrumb">
					<a href="<?php echo $this->Gym->createurl("MembershipPayment","generatePaymentInvoice");?>" class="btn btn-flat btn-custom"><i class="fa fa-bars"></i> <?php echo __("Gerar fatura de pagamento");?></a>
				</ol>
			<?php } ?>
			</section>
		</div>
		...
		<div class="box-body">
		 <table id="payment_list" class="table table-striped" cellspacing="0" width="100%">
        	<thead>
            <tr>
				<th><?php  echo __( 'Plano', 'gym_mgt' ) ;?></th>
				<th><?php  echo __( 'Nome', 'gym_mgt' ) ;?></th>
				<th><?php  echo __( 'Preço', 'gym_mgt' ) ;?></th>
				<th><?php  echo __( 'Valor pago', 'gym_mgt' ) ;?></th>
				<th><?php  echo __( 'Valor pendente', 'gym_mgt' ) ;?></th>
				<th><?php  echo __( 'Data de Início', 'gym_mgt' ) ;?></th>
				<th><?php  echo __( 'Data de Término', 'gym_mgt' ) ;?></th>
				<th><?php  echo __( 'Status', 'gym_mgt' ) ;?></th>
				<th><?php  echo __( 'Ação', 'gym_mgt' ) ;?></th>
            </tr>
			</thead>
			<tbody>
				<?php
					...
					if(!empty($data)) {
						foreach($data as $row) {
							...
							echo "<tr>
									...
									<td><span class='bg-primary pay_status'>". __($this->Gym->get_membership_paymentstatus($row['mp_id']) == "Fully Paid"? "Pago": "Não pago")."<span></td>
									"; 
									if($due == 0) {
										echo "
										<td>
										<a href='javascript:void(0)' class='btn1 btn btn-flat btn-default amt_pay' disabled data-url='".$this->request->base ."/GymAjax/gymPay/{$row['mp_id']}/{$due}'>".__('Pagar')."</a>
										...
										echo "
										<td>
										<a href='javascript:void(0)' class='btn1 btn btn-flat btn-default amt_pay' data-url='".$this->request->base ."/GymAjax/gymPay/{$row['mp_id']}/{$due}'>".__('Pagar')."</a>
										<a href='javascript:void(0)' class='btn1 btn btn-flat btn-info view_invoice' data-url='".$this->request->base ."/GymAjax/viewInvoice/{$row['mp_id']}'><i class='fa fa-eye'></i></a>";
										if($session["role_name"] == "administrator" || $session["role_name"] == "staff_member") {
											echo " <a href='".$this->request->base ."/MembershipPayment/MembershipEdit/{$row['mp_id']}' class='btn1 btn btn-flat btn-primary' title='Edit'><i class='fa fa-edit'></i></a>
											<a href='".$this->request->base ."/MembershipPayment/deletePayment/{$row['mp_id']}' class='btn1 btn btn-flat btn-danger' onclick=\"return confirm('$confirmMsg')\"><i class='fa fa-trash'></i></a>";
