...
{		
		...
		if(classes == null)
		{
			alert("Primeiro selecione uma atividade");
			return false;
		}else{
			return true;
		}		
	...
	<div class="col-md-12 box box-default">		
		<div class="box-header">
			<section class="content-header">
			  <h1>
				<?php echo __("Atividades no Plano");?>
			  </h1>
			  <ol class="breadcrumb">
				<a href="<?php echo $this->Gym->createurl("Membership","membershipList");?>" class="btn btn-flat btn-custom"><i class="fa fa-bars"></i> <?php echo __("Voltar");?></a>
			  </ol>
			</section>
		</div>
		<hr>
		<div class="box-body">
			...
			echo $this->Form->label(__("Selecione a atividade"))."&nbsp;";
			echo "</div>";
			...
			<table class="mydataTable table table-striped table-responsive" "width:100%;">
				<thead>
					<tr>
						<th><?php echo __("Nome da Atividade");?></th>
						<th><?php echo __("Categoria da Atividade");?></th>
						<th><?php echo __("Instrutor de atividades");?></th>						
				...
				<?php
					foreach($assigned_activities as $activity)
					{
						...
						echo "<tr>
							...
							<td>{$category}</td>
							<td>{$staff}</td>
							<td><a href='".$this->Gym->createurl('Membership','deleteActivity')."/{$activity['id']}' class='btn btn-flat btn-danger'>".__('Excluir')."</a></td>
							</tr>";
					}
				?>
				</tbody>
